    ...
    if (props?.audio) {
        return (
            <TouchableOpacity
                ...
                activeOpacity={0.9}
                style={{ width: width - 10, marginBottom: 15, flexDirection: 'row', alignItems: 'center' }}
            >
                <ImageBackground source={require('./../../assets/images/sermons-01.jpeg')} style={{ width: 80, height: 80, marginRight: 13, marginBottom: 5, borderRadius: 10, overflow: 'hidden', justifyContent: 'center', alignItems: 'center' }}>
                ...
                <View>
                    <Text style={{ fontFamily: fonts.primary, fontSize: 11, textAlign: 'left', color: '#444', marginBottom: isRTL ? 5 : 0 }}>{moment(parseInt(1691195928528)).format("DD MMM, YYYY")}</Text>
                    <Text style={{ fontFamily: isRTL ? fonts.arabicBold : fonts.primarySemiBold, fontSize: isRTL ? 15 : 14, textAlign: 'left', marginBottom: isRTL ? 5 : 0 }}>{strings.posttitle}</Text>
                    <Text style={{ fontFamily: isRTL ? fonts.arabicRegular : fonts.primary, fontSize: isRTL ? 12 : 11, textAlign: 'left', lineHeight: isRTL ? 17 : 16 }} numberOfLines={2}>{strings.postdesc}</Text>
                </View>
            </TouchableOpacity>
        )
    } else {
        return (
            <TouchableOpacity
                onPress={() => {
                    props?.video && navigation.navigate('VideoDetail', { item: itemobject })
                    props?.document && navigation.navigate('PDFView', { file: itemobject })
                    props?.image && navigation.navigate('ImageDetail', { item: itemobject })
                }}
                activeOpacity={0.9}
