    ...
    return (
        ...
        >
            <ImageBackground
                ...
                style={[itemstyle.otherimage, { height: 60, width: 60, alignItems: 'flex-end', justifyContent: 'flex-end' }]}
            >
                <View style={{ position: 'absolute', left: 0, top: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0, 0, 0, 0.5)' }} />
                <View style={[itemstyle?.iconbg, { width: 25, height: 25, borderRadius: 0, borderTopLeftRadius: 13 }]}>
                    <Icon name="book" style={[itemstyle?.icon, { fontSize: 10 }]} />
                </View>
            </ImageBackground>
            {/* <Text style={itemstyle.date}>{moment(parseInt(1691195928528)).format("DD MMM, YYYY")}</Text> */}
            <View style={{marginLeft: 10}}>
            <Text numberOfLines={1} style={itemstyle.title}>{item?.title}</Text>
...
const itemstyle = StyleSheet.create({
    ...
    title: { fontFamily: isRTL ? fonts.arabicBold : fonts.primarySemiBold, fontSize: 16, color: isDarkMode ? colors.black : colors.black, marginBottom: isRTL ? 7 : 0 , width: width - 100},
    desc: { fontFamily: isRTL ? fonts.arabicRegular : fonts.primary, fontSize: isRTL ? 13 : 12, color: isDarkMode ? colors.black : colors.black, lineHeight: isRTL ? 17 : 16 },
    ...
    otherimage: {  borderRadius: 10, overflow: 'hidden', justifyContent: 'center', alignItems: 'center' },
